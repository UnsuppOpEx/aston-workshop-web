...
class UserControllerTest {
  ...
  void createUser() throws Exception {
    UserResponse response =
        new UserResponse(userId, "John", "john@test.com", 30, LocalDateTime.now());
  ...
  void getUserById() throws Exception {
    when(userService.getUser(userId))
        .thenReturn(new UserResponse(userId, "Alex", "alex@test.com", 25, LocalDateTime.now()));
  ...
  void getAllUsers() throws Exception {
    when(userService.getAllUsers())
        .thenReturn(
            List.of(
                new UserResponse(
                    UUID.randomUUID(), "Alex", "alex@test.com", 20, LocalDateTime.now()),
                new UserResponse(
                    UUID.randomUUID(), "Ben", "ben@test.com", 30, LocalDateTime.now())));
  ...
  void updateUser() throws Exception {
    when(userService.updateUser(eq(userId), any()))
        .thenReturn(
            new UserResponse(userId, "Alex Junior", "alex@test.com", 10, LocalDateTime.now()));
